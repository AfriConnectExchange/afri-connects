...
import { CheckCircle, Package, Clock, Shield, Handshake, Truck, Download, Share, Home } from 'lucide-react';
      ...
      {paymentData.paymentMethod === 'escrow' && (
        <Alert>
          ...
          <AlertDescription>
            ...
            receipt of your order. If there are any issues, you can raise a dispute within 7 days 
            of delivery for full protection.
          </AlertDescription>
        </Alert>
      )}

      {paymentData.paymentMethod === 'barter_proposal' && (
        <Alert>
          <Handshake className="h-4 w-4" />
          <AlertDescription>
            <strong>Barter Exchange:</strong> The seller has up to {' '}
            {Math.ceil((new Date(paymentData.expiresAt).getTime() - Date.now()) / (1000 * 60 * 60 * 24))} days 
            to respond to your proposal. You'll receive a notification with their decision.
          </AlertDescription>
        </Alert>
      )}
