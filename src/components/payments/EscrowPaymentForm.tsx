...
export function EscrowPaymentForm({ orderTotal, onConfirm, onCancel }: EscrowPaymentFormProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.agreeEscrowTerms) {
      newErrors.escrowTerms = 'Please agree to escrow terms';
    }
    if (!formData.agreePaymentTerms) {
      newErrors.paymentTerms = 'Please agree to payment terms';
    }
          ...
          <div className="space-y-3">
            <div className="flex items-start space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => handleInputChange('agreeEscrowTerms', checked ? 'true' : 'false')}
              />
            ...
            {errors.escrowTerms && <p className="text-destructive text-sm">{errors.escrowTerms}</p>}

            <div className="flex items-start space-x-2">
              <Checkbox
                id="paymentTerms"
                checked={formData.agreePaymentTerms}
                onCheckedChange={(checked) => handleInputChange('agreePaymentTerms', checked ? 'true' : 'false')}
              />
            ...
            {errors.paymentTerms && <p className="text-destructive text-sm">{errors.paymentTerms}</p>}
          </div>
        </div>
